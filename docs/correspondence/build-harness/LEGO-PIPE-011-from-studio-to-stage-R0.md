...
to: ["ChatGPT (research/review)", "Claude Code (repo, after reconciliation)"]
cluster: play-well
repos:
  - ojfbot/lego-village-pipeline   # code (exists)
  - ojfbot/play-well-library       # content (to create)
supersedes: LEGO-PIPE-009-R1
...
finding_ids: "R-nn (shared ledger with the LEGO-PIPE-007 design-bundle review)"
tags:
  - bricklink-studio
  - io-file-format
...
*R0, for review · 2026-09-17 · play-well cluster · supersedes/extends LEGO-PIPE-009-R1; ADRs 0001–0005 remain in force*

**From:** Claude (Cowork) with James · **To:** ChatGPT (research/review) → after reconciliation, Claude Code (repo)
...
**ChatGPT (now):** answer §2 with P0/P1/P2 severities and `R-nn` IDs; challenge anything else you see; propose concrete alternatives, not just objections. Return as a findings list keyed to the question numbers, plus "unasked" findings at the end, and a "disagreements worth registering" section for places you'd argue the other side without calling it a defect.

**James + Claude (reconciliation):** fold accepted findings into Parts C/D as amendments, bump this memo to **R1**, and re-issue.

**Claude Code (after R1):**
- Scaffold `brickcore/` in `ojfbot/lego-village-pipeline`: move mils-integrator's `ldraw/parser.py` + `catalog.py` into it (shared, not forked); add `frames`, `lattice`, `connectors`, `graph`, `collision`, `inventory`, `diff`, `io.ldraw`, `io.io` (Studio zip), `io.usd` (pxr-only module), `validate`, `cli`.
- Create `ojfbot/play-well-library` with the layout in Part D §3.2 (USD variant), an attribution file for LDraw/LDCad data, LFS for thumbnails, and the branch flow rules in CI (play → staging → main only).
- Run S2 and S3 in CI against OMR fixtures; write findings notes into the project docs.
- Keep `ValidationReport` and `DiffOp` shared with mils-integrator and roofsnow.
- Register both repos in Northstar under play-well.

**Claude (Cowork, on the Mac):** S1, S4, S5, S6, S7.

## 4. Spikes (each ends in a short findings note in the project)

| Spike | Question | Done when |
|---|---|---|
| S1 Studio round trip | Minimum `.io` entry set Studio 2.26 accepts; what it normalises open→save; does any metadata survive; `open` behaviour with Studio running | Table of normalisations; writer produces files Studio opens clean |
| S2 Connector extraction | Primitive→connector table; shadow-lib coverage on ~10 Winter Village OMR sets + 41843 + 10254 | ≥90 % of part instances resolved |
| S3 Collision | Surface-voxel rules vs BVH on the same sets | 0 false positives on known-goods after stud-strip + face-contact rules |
| S4 Blender adapter | Importer choice (ImportLDraw GPL vs ldr_tools MIT), scale constant, instancing at thousands of parts, headless startup + render time | Numbers recorded; frames round trip exact on lattice points |
| S5 Oracle readout | Can computer use read Studio's check colouring reliably on synthetic mutations? | Accuracy vs known answers ≥ target set in R1 |
| S6 Composition round trip (USD) | Two published buildings + riser + snow layer + season variant → module → flatten → Studio → hand edit → ingest as diff on the right asset | Diff lands on the asset, not the flattened module |
| S7 `pxr` everywhere | `usd-core` native on macOS; arm64 Linux container via community wheel or source build; Blender imports the composed stage | Both environments load the same stage |
...
# PART D — build-harness/storage-architecture-asset-versioning.md
*In which the folder-of-files is abandoned; the studios' work/publish split, product/version/representation and composition-by-reference are adopted; two repos and a blob store are proposed; `lvp://` URIs and a branch flow replace file locking; and OpenUSD is elected canonical under five guardrails, with its costs priced and its learning path laid out (ADR 0012).*
